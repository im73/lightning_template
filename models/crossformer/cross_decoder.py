...
class NewDecoder(nn.Module):
    ...
    def __init__(self, in_dim, out_dim, dropout):
        ...
        self.decoder_layer1 = nn.Linear(in_dim, in_dim)
        self.batchnorm = nn.LayerNorm(in_dim)
        self.decoder_layer2 = nn.Linear(in_dim, out_dim)
        self.activation = nn.Sigmoid()
        self.dropout = nn.Dropout(dropout)
        
    def forward(self, x):
        new_x = x
        x = new_x + self.dropout(self.decoder_layer1(x))
        out = self.activation(self.decoder_layer2(x))
        return out
